...
import { Eye, Video, Megaphone, Anchor, TrendingUp, Activity, Users, UserPlus, Globe, HeartHandshake } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white selection:bg-brand-orange/30 font-sans pt-24 md:pt-32 pb-20 px-4 md:px-6 relative overflow-hidden">
      ...
      >
        ...
        <motion.h1 variants={fadeInUp} className="text-4xl md:text-7xl font-medium tracking-tighter mb-6 md:mb-8 leading-tight">
          The Engine <span className="text-zinc-500">&</span> <br />
          <span className="text-transparent bg-clip-text bg-gradient-to-r from-brand-orange to-brand-yellow">The Architects.</span>
        </motion.h1>
        
        <motion.p variants={fadeInUp} className="text-lg md:text-xl text-zinc-400 font-light max-w-3xl mx-auto leading-relaxed">
          A strong leadership backbone ensures coordination, community building, and content excellence.
        </motion.p>
      </motion.div>
      ...
      <section className="max-w-7xl mx-auto mb-24 md:mb-40 relative z-10">
         ...
         >
            ...
            <h2 className="text-2xl md:text-3xl font-medium">The Ecosystem Triad</h2>
         </motion.div>
         ...
         <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
            {companies.map((company, i) => (
               <SpotlightCard key={i} delay={i * 0.1}>
                  ...
                  <span className="text-zinc-500 font-mono text-[10px] md:text-xs uppercase tracking-widest mb-2 block">Entity 0{i + 1}</span>
                  <h3 className="text-xl md:text-2xl font-bold mb-3 md:mb-4">{company.title}</h3>
                  <p className="text-zinc-400 text-sm md:text-base leading-relaxed mb-6 md:mb-8 flex-grow">{company.desc}</p>
                  
                  <div className="pt-6 md:pt-8 border-t border-white/5">
                     <span className="text-[10px] md:text-xs font-bold uppercase text-white/50 mb-3 block">Focus Areas</span>
                     <div className="flex flex-wrap gap-2">
                        {company.tags.map(tag => (
                           <span key={tag} className="px-2 py-1 md:px-3 rounded-full bg-white/5 text-[10px] uppercase tracking-wider text-zinc-300 border border-white/5">
                              {tag}
                           </span>
                        ))}
                     </div>
      ...
      <section className="max-w-7xl mx-auto mb-24 md:mb-32 relative z-10">
         <div className="mb-10 md:mb-16 flex flex-col md:flex-row md:items-end justify-between gap-4 md:gap-6">
            ...
            >
                ...
                <div>
                    <h2 className="text-2xl md:text-3xl font-medium">The Leadership Core</h2>
                    <p className="text-zinc-400 text-xs md:text-sm mt-1">Visionaries & Strategists</p>
                </div>
            </motion.div>
         </div>
               ...
               <SpotlightCard key={i} delay={i * 0.1} className="h-full">
                  <div className="flex flex-col h-full">
                     ...
                     <div className="flex justify-between items-start mb-6 md:mb-8">
                        <span className={`px-2 py-1 md:px-3 md:py-1.5 rounded-full text-[9px] md:text-[10px] font-bold uppercase tracking-widest border ${member.tag === 'Co-Founder' ? 'bg-brand-orange/10 text-brand-orange border-brand-orange/20' : 'bg-white/5 text-zinc-400 border-white/10 group-hover:text-white group-hover:border-white/20 transition-colors'}`}>
                           {member.tag}
                        </span>
                        <member.icon size={18} className="text-zinc-700 group-hover:text-brand-yellow transition-colors duration-300 md:w-5 md:h-5" />
                     </div>

                     {/* BODY */}
                     <div className="mb-4 md:mb-6">
                        <h3 className="text-3xl md:text-4xl font-medium text-white mb-1 tracking-tight">{member.name}</h3>
                        <p className="text-brand-yellow text-[10px] md:text-xs font-bold uppercase tracking-widest">{member.role}</p>
                     </div>
                     ...
                     <div className="mt-auto border-t border-white/5 pt-4 md:pt-6">
                        <ul className="space-y-2 md:space-y-3">
                           {member.points.map((point, idx) => (
                              <li key={idx} className="flex items-start gap-3 text-xs md:text-sm text-zinc-400 font-light leading-snug group-hover:text-zinc-300 transition-colors">
      ...
      <section className="max-w-7xl mx-auto mb-24 md:mb-40 relative z-10">
         <div className="mb-8 md:mb-12 flex items-center gap-4">
            ...
            <div>
                <h2 className="text-2xl md:text-3xl font-medium">The Ambassadors</h2>
                <p className="text-zinc-400 text-xs md:text-sm mt-1">Tier & Regional Leads</p>
            </div>
         </div>
         ...
         <SpotlightCard className="w-full">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 p-2 md:p-4">
               ...
               <div>
                  ...
                  <h3 className="text-2xl md:text-3xl font-medium mb-3 md:mb-4">The Extension of the Core.</h3>
                  <p className="text-zinc-400 text-sm md:text-base leading-relaxed mb-6">
                     Ambassadors are the vital link between the leadership and the community. 
                     They represent Inner Circle across regions, driving participation and growth on the ground.
                  </p>
                  <div className="flex flex-wrap gap-2">
                     {["Regional Leads", "Tier Leads", "Mentors", "Hosts"].map(tag => (
                        <span key={tag} className="px-2 py-1 md:px-3 rounded-full border border-white/10 bg-white/5 text-[10px] md:text-xs uppercase tracking-widest text-zinc-400">
                           {tag}
                        </span>
                     ))}
                  </div>
               ...
               <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {ambassadorDuties.map((duty, i) => (
                     <div key={i} className="p-4 rounded-2xl bg-black/40 border border-white/5 hover:border-brand-orange/20 transition-colors">
                        <duty.icon size={18} className="text-brand-yellow mb-3 md:w-5 md:h-5" />
                        <h4 className="text-xs md:text-sm font-bold text-white mb-1">{duty.title}</h4>
                        <p className="text-[10px] md:text-xs text-zinc-500">{duty.desc}</p>
                     </div>
                  ))}
               </div>
