...
export default function MerchPage() {
  ...
  return (
    <div ref={containerRef} className="min-h-screen bg-[#050505] text-white selection:bg-brand-orange/30 font-sans pt-32 pb-20 px-6 relative">
      ...
      >
        ...
        <h1 className="text-5xl md:text-8xl font-medium tracking-tighter mb-8">
          Wear Your <br />
          <span className="text-transparent bg-clip-text bg-gradient-to-r from-brand-orange to-brand-yellow">Legacy.</span>
        </h1>
        
        <p className="text-xl text-zinc-400 font-light max-w-2xl mx-auto leading-relaxed">
          The official uniform of the Inner Circle ecosystem. Designed for visibility, connection, and impact.
        </p>
      </motion.div>
      ...
      <section className="max-w-7xl mx-auto mb-32">
         <div className="relative h-[300px] md:h-[400px] rounded-[3rem] overflow-hidden bg-gradient-to-r from-zinc-900 to-black border border-white/5 flex items-center justify-center group">
            ...
            <div className="text-center z-10">
               ...
               <h3 className="text-3xl md:text-5xl font-bold tracking-tighter">Built for the Architects.</h3>
            </div>
         </div>
      </section>
...
function CarouselSection({ title, subtitle, items, isSmall = false }: { title: string, subtitle: string, items: any[], isSmall?: boolean }) {
    ...
    <section className="max-w-[100vw] mb-32 relative group/carousel">
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-0 mb-8 flex items-end justify-between border-b border-white/10 pb-6">
            <div className="flex flex-col">
                ...
                <span className="text-zinc-500 text-sm font-mono uppercase tracking-widest mt-2">{subtitle}</span>
            </div>
        </div>
        ...
        <div className="relative">
            ...
            >
                {items.map((item, index) => (
                    <div 
                        key={index} 
                        className={`snap-start shrink-0 ${isSmall ? 'w-[280px]' : 'w-[320px] md:w-[380px]'}`}
                    >
...
function ShowcaseCard({ item, isSmall = false }: { item: any, isSmall?: boolean }) {
   return (
      <motion.div
         whileHover={{ y: -10 }}
      ...
      >
         ...
         <div className={`relative w-full bg-[#F4F4F5] rounded-[2.5rem] overflow-hidden mb-6 ${isSmall ? 'aspect-square' : 'aspect-[4/5]'} flex items-center justify-center`}>
         ...
         {/* MINIMAL TEXT */}
         <div className="px-2">
            <h3 className="text-2xl font-bold text-white group-hover:text-brand-orange transition-colors">{item.name}</h3>
