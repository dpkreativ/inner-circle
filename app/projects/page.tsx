...
export default function ProjectsPage() {
  ...
  return (
    <div
      ref={containerRef}
      className="min-h-screen bg-background text-foreground selection:bg-brand-orange/30 font-sans pt-32 pb-20 px-6 relative overflow-hidden"
    >
      {/* --- BACKGROUND GRID --- */}
      ...
      >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-end">
          <motion.h1
            variants={fadeInUp}
            className="text-5xl md:text-7xl font-medium tracking-tighter leading-tight text-foreground"
          >
            Building the <br />
          ...
          <motion.p
            variants={fadeInUp}
            className="text-xl text-muted-foreground font-light leading-relaxed pb-2"
          >
      ...
      <section className="max-w-7xl mx-auto mb-40 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-2">
            <h3 className="text-2xl font-medium mb-12 flex items-center gap-3 text-foreground">
              <Target className="text-brand-orange" />
              Key Milestones
            </h3>

            <div className="relative border-l border-border dark:border-white/10 ml-4 space-y-16">
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-32">
              <div className="p-8 rounded-[2rem] bg-card/50 dark:bg-zinc-900/50 border border-border dark:border-white/10 backdrop-blur-md">
                ...
                {/* Calendar Visual */}
                <div className="grid grid-cols-7 gap-2 mb-8 text-center text-sm font-mono text-muted-foreground">
                  <span>M</span>
                  <span>T</span>
                  <span>W</span>
                  <span>T</span>
                ...
                <div className="space-y-4">
                  <div className="flex gap-4 items-start p-4 rounded-xl bg-muted/50 dark:bg-white/5 border border-border dark:border-white/5">
      ...
      <section className="max-w-7xl mx-auto pb-32 relative z-10">
        <div className="flex items-end justify-between mb-12 border-b border-border dark:border-white/10 pb-6">
...
function TimelineItem({ item, delay }: { item: MilestoneItem; delay: number }) {
  return (
    ...
    >
      ...
      <div
        className={`absolute left-[-5px] top-2 w-2.5 h-2.5 rounded-full border-2 ${
          item.status === "done"
            ...
            : "dark:bg-black bg-white border-zinc-300 dark:border-zinc-600"
        }`}
      />
      ...
      <p className="text-muted-foreground dark:text-zinc-400 leading-relaxed max-w-lg">
        {item.desc}
      </p>
    </motion.div>
  );
...
function ProjectCard({
  ...
  return (
    <motion.div
      ...
      transition={{ delay }}
      whileHover={{ y: -5 }}
      className="group relative p-8 rounded-[2.5rem] bg-card/50 dark:bg-zinc-900/30 border border-border dark:border-white/5 hover:border-brand-orange/30 hover:bg-card dark:hover:bg-zinc-900 transition-all duration-500 cursor-pointer overflow-hidden"
    >
      <div className="absolute top-0 right-0 p-8 opacity-20 group-hover:opacity-100 transition-opacity">
        <ArrowRight className="text-foreground -rotate-45 group-hover:rotate-0 transition-transform duration-500" />
      </div>

      <div className="w-12 h-12 rounded-full bg-muted dark:bg-white/5 flex items-center justify-center mb-8 text-muted-foreground dark:text-zinc-400 group-hover:text-brand-yellow group-hover:bg-brand-yellow/10 transition-colors">
        <project.icon size={24} />
      </div>

      <span className="inline-block px-3 py-1 rounded-full border border-border dark:border-white/10 bg-muted dark:bg-white/5 text-[10px] uppercase tracking-widest text-muted-foreground dark:text-zinc-400 mb-4">
        {project.tag}
      </span>
