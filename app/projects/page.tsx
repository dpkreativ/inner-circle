...
import { Calendar, Clock, MapPin, ArrowRight, Radio, Zap, Users, Target, ChevronRight, Play } from "lucide-react";
...
export default function ProjectsPage() {
  ...
  return (
    <div ref={containerRef} className="min-h-screen bg-[#050505] text-white selection:bg-brand-orange/30 font-sans pt-32 pb-20 px-6 relative overflow-hidden">
      ...
      >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-end">
            <motion.h1 variants={fadeInUp} className="text-5xl md:text-7xl font-medium tracking-tighter leading-tight">
              Building the <br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-brand-orange to-brand-yellow">Next Chapter.</span>
            </motion.h1>
            <motion.p variants={fadeInUp} className="text-xl text-zinc-400 font-light leading-relaxed pb-2">
              From digital storytelling to physical summits. Explore the active initiatives defining the Inner Circle ecosystem [cite: 161-168].
            </motion.p>
        </div>
      </motion.div>
                           ...
                           return (
                              <div 
                                 ...
                                 className={`
                                    ...
                                    ${isEvent ? 'text-brand-orange font-bold' : ''}
                                 `}
                              >
                                 {day}
                                 {isEvent && <div className="absolute bottom-1 w-1 h-1 rounded-full bg-brand-orange" />}
                              </div>
                           )
                        })}
                     </div>
                     ...
                     <div className="space-y-4">
                        <div className="flex gap-4 items-start p-4 rounded-xl bg-white/5 border border-white/5">
                           <div className="text-center min-w-[3rem]">
                              <span className="block text-xs text-brand-orange uppercase font-bold">Dec</span>
                              <span className="block text-xl font-bold">13</span>
                           </div>
                           <div>
                              <h4 className="font-bold text-sm">CircleConnect 1.0</h4>
                              <p className="text-xs text-zinc-500 mt-1">Official Launch Event </p>
                           </div>
                        </div>
                        
                        <div className="flex gap-4 items-start p-4 rounded-xl bg-white/5 border border-white/5">
                           <div className="text-center min-w-[3rem]">
                              <span className="block text-xs text-zinc-500 uppercase font-bold">Jan</span>
                              <span className="block text-xl font-bold text-zinc-400">01</span>
                           </div>
                           <div>
                              <h4 className="font-bold text-sm text-zinc-400">Monthly Calendar</h4>
                              <p className="text-xs text-zinc-600 mt-1">2026 Series Begins </p>
                           </div>
                        </div>
                     </div>
      ...
      <section className="max-w-7xl mx-auto pb-32 relative z-10">
         <div className="flex items-end justify-between mb-12 border-b border-white/10 pb-6">
            ...
            <span className="text-zinc-500 font-mono text-xs uppercase tracking-widest">Current Phase</span>
         </div>
...
function TimelineItem({ item, delay }: { item: any, delay: number }) {
   return (
      ...
      >
         {/* Dot on line */}
         <div className={`absolute left-[-5px] top-2 w-2.5 h-2.5 rounded-full border-2 ${item.status === 'done' ? 'bg-brand-orange border-brand-orange' : 'bg-black border-zinc-600'}`} />
         
         <span className={`text-xs font-mono uppercase tracking-widest mb-2 block ${item.status === 'done' ? 'text-brand-orange' : 'text-zinc-500'}`}>
            {item.date}
         </span>
...
function ProjectCard({ project, delay }: { project: any, delay: number }) {
   return (
      ...
      >
         ...
         <h3 className="text-3xl font-bold mb-3 group-hover:text-brand-orange transition-colors">{project.title}</h3>
